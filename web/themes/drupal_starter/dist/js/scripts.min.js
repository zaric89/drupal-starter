...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _jsFormValidate = __webpack_require__(5);

	var _jsFormValidate2 = _interopRequireDefault(_jsFormValidate);
	...
	jQuery(function () {
		...
		_jsFormValidate2.default.init();

		//Messages
		_messages2.default.init();
	 ...
	 -------------------------------------------------------------------------------*/
		init: function init() {
			...
			var closeNav = function closeNav() {
				...
				Menu.$domMenuNav.removeClass(Menu.classOpen).find(Menu.classSubMenu).slideUp();
				Menu.$domMenuNav.find(Menu.classSubIcon).removeClass(Menu.classOpen);
				_global2.default.$domBody.removeClass(Menu.classOpenMenu);
			};

			if (_global2.default.varsWindowWidth < 992) {
				Menu.$domMenuHasSub.each(function (i, el) {
					$(el).append('<span class="sub-icon font-plus-circle" data-open-sub></span>');
				});
			}

			// bind events
			Menu.$domMenuBtn.on('click', function (e) {
				e.preventDefault();
				Menu.$domMenuBtn.toggleClass(Menu.classOpen);
				Menu.$domMenuNav.toggleClass(Menu.classOpen).find(Menu.classSubMenu).slideUp();
				Menu.$domMenuNav.find(Menu.classSubIcon).removeClass('open');
				_global2.default.$domBody.toggleClass(Menu.classOpenMenu);
			});

			Menu.$domMenuNav.on('click', '[data-open-sub]', function (e) {
				var self = $(e.currentTarget);

				if (self.hasClass(Menu.classOpen)) {
					self.removeClass(Menu.classOpen).siblings(Menu.classSubMenu).slideUp();
					self.siblings().find(Menu.classSubIcon).removeClass(Menu.classOpen);
					self.siblings().find(Menu.classMenu).slideUp();
				} else {
					self.addClass(Menu.classOpen).siblings(Menu.classSubMenu).slideDown();
					self.parent().siblings(Menu.classList).find(Menu.classSubIcon).removeClass(Menu.classOpen);
					self.parent().siblings(Menu.classList).find(Menu.classMenu).slideUp();
				}
			});

			_global2.default.functions.clickOutsideContainer(Menu.$domMenuNav, Menu.$domMenuNav.find('ul'), Menu.$domMenuBtn, closeNav);
...
/***/ (function(module, exports) {
	...
	var Messages = {
	    ...
	    init: function init() {
	        Messages.$statusMessages.on('click', function (e) {
	            var targetMessage = $(e.currentTarget);
	            targetMessage.addClass('hide');
	        });

	        Messages.$errorMessages.on('click', function (e) {
	            var targetMessage = $(e.currentTarget);
	            targetMessage.addClass('hide');
	        });

	        Messages.$warningMessages.on('click', function (e) {
	            var targetMessage = $(e.currentTarget);
	            targetMessage.addClass('hide');
	        });
	    }
	...
	var Select2 = {
	 ...
	 -------------------------------------------------------------------------------*/
		init: function init() {
			...
			var selectPlaceholder = Drupal.t("Select an option");
